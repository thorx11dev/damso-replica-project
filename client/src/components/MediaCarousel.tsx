...
export const MediaCarousel = forwardRef<{ navigate: (direction: "prev" | "next") => void }>((props, ref) => {
  ...
  const getItemPosition = (index: number) => {
    ...
    const position = diff * 280;
    const scale = index === currentIndex ? 1 : 0.5;
  ...
  return (
    <div className="relative w-full h-[350px] flex items-center justify-center overflow-hidden perspective-[1200px]">
      <div className="relative w-full h-full flex items-center justify-center">
        {mediaItems.map((item, index) => (
          ...
          >
            <div className="w-[280px] h-[280px] bg-card rounded-lg overflow-hidden shadow-2xl border border-border hover:shadow-[0_0_40px_rgba(0,0,0,0.3)] transition-shadow duration-300 group">
              <img 
                src={item.src} 
                alt={item.alt}
